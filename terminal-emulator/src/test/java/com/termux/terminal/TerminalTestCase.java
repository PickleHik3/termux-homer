...
>>>>>>> upstream/master
    ...
    public void assertHistoryStartsWith(String... rows) {
        assertTrue("About to check " + rows.length + " lines, but only " + mTerminal.getScreen().getActiveTranscriptRows() + " in history", mTerminal.getScreen().getActiveTranscriptRows() >= rows.length);
    ...
    protected void assertLineIs(int line, String expected) {
        ...
        if (textLen != expected.length())
            fail("Expected '" + expected + "' (len=" + expected.length() + "), was='" + new String(chars, 0, textLen) + "' (len=" + textLen + ")");
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            for (int j = 0; j < line.length; j++) {
                ...
                if ((effectsAtCell & attributes) != attributes)
                    fail("Line=" + i + ", column=" + j + ", expected " + describeStyle(attributes) + " set, was " + describeStyle(effectsAtCell));
            }
        }
        return this;
    }
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            for (int j = 0; j < line.length; j++) {
                ...
                if (actualColor != expectedColor)
                    fail("Line=" + i + ", column=" + j + ", expected color " + Integer.toHexString(expectedColor) + " set, was " + Integer.toHexString(actualColor));
            }
        }
        return this;
    }
